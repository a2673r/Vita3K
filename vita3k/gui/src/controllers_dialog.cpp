...
void add_bind_to_table(GuiState &gui, EmuEnvState &emuenv, const SDL_GameControllerType type, const SDL_GameControllerButton btn) {
...
void draw_controllers_dialog(GuiState &gui, EmuEnvState &emuenv) {
    ...
    if (ctrl.controllers_num)
        ImGui::SetNextWindowSize(ImVec2(VIEWPORT_SIZE.x / 2.5f, VIEWPORT_SIZE.y / 2.7f), ImGuiCond_Always);
    ...
    if (ctrl.controllers_num) {
        ...
        if (ImGui::BeginTable("main", 2, ImGuiTableFlags_NoSavedSettings | ImGuiTableFlags_BordersInnerV)) {
            ...
            for (auto i = 0; i < ctrl.controllers_num; i++) {
                ...
                if (rebinds_is_open) {
                    ImGui::SetNextWindowSize(ImVec2(VIEWPORT_SIZE.x / 1.4f, VIEWPORT_SIZE.y / 1.35f), ImGuiCond_Always);
                    ImGui::SetNextWindowPos(ImVec2(VIEWPORT_POS.x + (VIEWPORT_SIZE.x / 2.f), VIEWPORT_POS.y + (VIEWPORT_SIZE.y / 2.f)), ImGuiCond_Always, ImVec2(0.5f, 0.5f));
                    ImGui::Begin("Rebind Controls", &rebinds_is_open, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings);
                    ImGui::SetWindowFontScale(RES_SCALE.x);
                    auto &controls = gui.lang.controls;
