...
bool Module::process(KernelState &kern, const MemState &mem, const SceUID thread_id, ModuleData &data, std::unique_lock<std::recursive_mutex> &scheduler_lock, std::unique_lock<std::mutex> &voice_lock) {
    ...
    if (static_cast<std::int32_t>(state->decoded_gran_pending) < data.parent->rack->system->granularity) {
        ...
        if (!data.extra_storage.empty()) {
            ...
            data.extra_storage.erase(data.extra_storage.begin(), data.extra_storage.begin() + state->decoded_gran_passed * 2 * sizeof(float));
        }
        ...
        while (static_cast<std::int32_t>(state->decoded_gran_pending) < data.parent->rack->system->granularity) {
            ...
            if (data.extra_storage.size() < sizeof(float) * 2 * data.parent->rack->system->granularity
                ...
                uint32_t samples_needed = data.parent->rack->system->granularity - state->decoded_gran_pending;
                ...
                if (params->playback_scalar != 1 || static_cast<int>(round(params->playback_frequency)) != data.parent->rack->system->sample_rate) {
                    ...
                    if (params->playback_scalar != 0)
                        src_sample_rate *= params->playback_scalar;

                    if (!state->swr) {
                        state->swr = swr_alloc_set_opts(nullptr,
                            AV_CH_LAYOUT_STEREO, AV_SAMPLE_FMT_FLT, static_cast<int>(data.parent->rack->system->sample_rate),
                            AV_CH_LAYOUT_STEREO, AV_SAMPLE_FMT_FLT, static_cast<int>(data.parent->rack->system->sample_rate * params->playback_scalar),
                            0, nullptr);
                    ...
                    const std::size_t current_count = state->decoded_gran_pending * sizeof(float) * 2;
    ...
    state->decoded_gran_pending -= gran_to_be_passed;
    state->decoded_gran_passed += gran_to_be_passed;
    state->samples_generated_since_key_on += gran_to_be_passed;
    state->samples_generated_total += gran_to_be_passed;
